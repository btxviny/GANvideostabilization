...
class GAN(keras.Model):
    ...
    def train_step(self, data): 
        ...
        with tf.GradientTape() as d1_tape, tf.GradientTape() as d2_tape, tf.GradientTape() as gen_tape:
            ...
            real_loss_d1 = tf.reduce_mean(tf.square(tf.ones_like(d1_real_predictions) - d1_real_predictions))
            fake_loss_d1 = tf.reduce_mean(tf.square(tf.zeros_like(d1_fake_predictions) - d1_fake_predictions))
            d1_loss = tf.add(real_loss_d1,fake_loss_d1)
            real_loss_d2 = tf.reduce_mean(tf.square(tf.ones_like(d2_real_predictions) - d2_real_predictions))
            fake_loss_d2 = tf.reduce_mean(tf.square(tf.zeros_like(d2_fake_predictions) - d2_fake_predictions))
